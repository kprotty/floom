...
impl<T: Send + Default + 'static> Receiver for floom::Receiver<T> {
    ...
    fn iter(&self) -> Box<dyn Iterator<Item=T> + '_> {
        Box::new(std::iter::from_fn(move || floom::Receiver::recv(self).ok()))
    }
}
        ...
        crossbeam_utils::thread::scope(|scope| {
            ...
            for i in 0..thread_num {
                ...
                scope.spawn(move |_| {
                    for _ in 0..msgs / thread_num as usize {
                        // if msgs >= 46 { println!("worker thread {} sending", i); }
                        tx.send(Default::default());
                    }
                    // if msgs >= 46 { println!("worker thread {} end", i); }
                });
            }
            ...
            scope.spawn(move |_| {
                ...
                for _ in 0..msgs - (msgs / thread_num as usize * thread_num)  {
                    // if msgs >= 46 { println!("main thread sending"); }
                    tx.send(Default::default());
                }
                // if msgs >= 46 { println!("main thread end"); }
            });

            for i in 0..msgs {
                // if msgs >= 46 { println!("receiving {}/{}", i, msgs); }
                black_box(rx.recv());
                // if msgs >= 46 { println!("received {}/{}", i + 1, msgs); }
            }
        })
            .unwrap();
...
fn kitsune_32t_1m(b: &mut Criterion) {
    ...
    b.bench_function("kitsune-32t-1m-floom", |b| test_kitsune::<floom::Sender<u32>>(b, 32, 1));
...
fn kitsune_32t_1000m(b: &mut Criterion) {
    ...
    b.bench_function("kitsune-32t-1000m-floom", |b| test_kitsune::<floom::Sender<u32>>(b, 32, 1000));
...
fn kitsune_256t_1m(b: &mut Criterion) {
    ...
    b.bench_function("kitsune-256t-1m-floom", |b| test_kitsune::<floom::Sender<u32>>(b, 256, 1));
...
fn kitsune_1t_1000m(b: &mut Criterion) {
    ...
    b.bench_function("kitsune-1t-1000m-floom", |b| test_kitsune::<floom::Sender<u32>>(b, 1, 1000));
...
fn kitsune_4t_10000m(b: &mut Criterion) {
    ...
    b.bench_function("kitsune-4t-10000m-floom", |b| test_kitsune::<floom::Sender<u32>>(b, 4, 10000));
...
fn robin_b_4t_1000m(b: &mut Criterion) {
    ...
    b.bench_function("robin-b-4t-1000m-floom", |b| test_robin_b::<floom::Sender<u32>>(b, 4, 1000));
...
fn mpsc_bounded_no_wait_4t(b: &mut Criterion) {
    ...
    b.bench_function("mpsc-bounded-no-wait-4t-floom", |b| test_mpsc_bounded_no_wait::<floom::Sender<u32>>(b, 4));
...
fn mpsc_bounded_4t(b: &mut Criterion) {
    for bound in &[0, 1, 10, 50, 10_000] {
        ...
        b.bench_function(&format!("{}{}", text, "floom"), |b| test_mpsc_bounded::<floom::Sender<u32>>(b, bound, 4));
