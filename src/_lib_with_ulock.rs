...
#[inline]
fn wait_lock<'a, T>(lock: &'a ChanLock<T>) -> LockGuard<'a, T> {
    lock.lock()
}
...
use usync::sync::{Lock, LockGuard};

struct ChanLock<T>(Lock<T>);

impl<T: Default> Default for ChanLock<T> {
    fn default() -> Self {
        Self::new(T::default())
    }
}
...
impl<T> ChanLock<T> {
    ...
    fn lock(&self) -> LockGuard<'_, T> {
        self.0.lock::<usync::parker::DefaultParker>()
    }
}
